...
.PHONY: rename
rename: ## Rename in all files from app_lang_selector to <new_name>: `make rename <new_name>`
	@if [ -z "$(filter-out $@,$(MAKECMDGOALS))" ]; then \
		...
		echo "\033[0;32mRenaming app_lang_selector to $$NEW_NAME in all files..."; \
		find . -type f \( -name "*.dart" -o -name "*.yaml" -o -name "*.yml" -o -name "*.md" -o -name "*.json" \) -not -path "./.dart_tool/*" -not -path "./build/*" -not -path "./.git/*" | xargs sed -i '' "s/app_lang_selector/$$NEW_NAME/g"; \
		echo "\033[0;32mRenaming lib/app_lang_selector.dart to lib/$$NEW_NAME.dart..."; \
		if [ -f "lib/app_lang_selector.dart" ]; then \
			mv "lib/app_lang_selector.dart" "lib/$$NEW_NAME.dart"; \
		fi; \
